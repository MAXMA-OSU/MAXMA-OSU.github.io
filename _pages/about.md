---
permalink: /
title: ""
excerpt: ""
author_profile: true
redirect_from: 
  - /about/
  - /about.html
---

{% if site.google_scholar_stats_use_cdn %}
{% assign gsDataBaseUrl = "https://cdn.jsdelivr.net/gh/" | append: site.repository | append: "@" %}
{% else %}
{% assign gsDataBaseUrl = "https://raw.githubusercontent.com/" | append: site.repository | append: "/" %}
{% endif %}
{% assign url = gsDataBaseUrl | append: "google-scholar-stats/gs_data_shieldsio.json" %}

<span class='anchor' id='about-me'></span>

I am Shuangxiu (Max) Ma, a Ph.D. candidate in the [William G. Lowrie Department of Chemical and Biomolecular Engineering](https://cbe.osu.edu/) at The Ohio State University, co-advised by Prof. Li-Chiang Lin (OSU / National Taiwan University) and Prof. Bhavik R. Bakshi (Arizona State University). I also completed a graduate minor in Computer Science and Engineering (machine learning track) at OSU.

My research sits at the intersection of molecular simulation, sustainability, and computational optimization. I develop multiscale simulation workflows — GCMC, classical and reactive molecular dynamics (ReaxFF), and QM/DFT — to connect atomistic mechanisms to macroscopic properties in polymers and porous materials, with applications in plastics recycling, olefin/paraffin separation membranes, and circular-economy process design. I also build machine-learning and Bayesian-optimization loops for materials screening, and hybrid quantum–classical optimization frameworks for multi-objective circular-economy problems.

My current interests include: design and simulation of complex DNA origami, reverse design of complex bio/polymer structures, machine-learning methods and benchmarks for optimization problems, and simulation pipelines for complex bio/chem systems.

I have published papers with total <a href='https://scholar.google.com/citations?user=tjls-loAAAAJ'>google scholar citations <strong><span id='total_cit'>1000+</span></strong></a> (h-index: 14) (You can also use google scholar badge <a href='https://scholar.google.com/citations?user=tjls-loAAAAJ'><img src="https://img.shields.io/endpoint?url={{ url | url_encode }}&logo=Google%20Scholar&labelColor=f6f6f6&color=9cf&style=flat&label=citations"></a>).


# 🔥 News
- *2025.11*: &nbsp;🎉 Three presentations at the 2025 AIChE Annual Meeting on PET degradation and sustainable process synthesis.
- *2025.07*: &nbsp;💼 Started a research/strategy internship at Mitsubishi Chemical Group (Japan) & The Global KAITEKI Center (USA).

# 🔬 Research Projects
- **EFRI E3P** — Sustainable and Circular Engineering for the Elimination of End-of-life Plastics: a framework for assessment, design, and innovation. (Advisors: Prof. Li-Chiang Lin, Prof. Bhavik R. Bakshi)
- **Facilitated Transport Membranes** — Development of facilitated transport membranes for olefin/paraffin separations. (Advisors: Prof. Li-Chiang Lin, Dr. Yang Han)
- **NEDO Challenge** — Quantum-assisted circular-economy multi-objective optimization framework. (Advisor: Dr. Qi Gao, Mitsubishi Chemical Corporation, Science & Innovation Center)

# 📝 Publications

Please see my full, up-to-date publication list on my <a href='https://scholar.google.com/citations?user=tjls-loAAAAJ'>Google Scholar profile</a> <a href='https://scholar.google.com/citations?user=tjls-loAAAAJ'><img src="https://img.shields.io/endpoint?url={{ url | url_encode }}&logo=Google%20Scholar&labelColor=f6f6f6&color=9cf&style=flat&label=citations"></a>.

# 🗣 Presentations
- S. Ma, B. Bakshi, L.-C. Lin. *Theoretical Assessment of PET Pyrolysis via Reactive Molecular Dynamics Simulation and Kinetic Modeling.* 2023 AIChE Annual Meeting.
- S. Ma, L.-C. Lin, B. Bakshi. *Exploring the Mechanism and Potential of Polystyrene–Zeolites Co-Pyrolysis System for Enhancing Styrene Recycling: A Molecular Dynamics Study.* 2024 AIChE Annual Meeting.
- S. Ma, P. Pereira, L.-C. Lin, P. E. Savage, B. Bakshi. *Understanding PET Fast Hydrolysis via Reactive Molecular Dynamics Simulation and Experimental Investigation.* 2025 AIChE Annual Meeting.
- S. Kim, S. Ma, P. Pereira, P. Savage, L.-C. Lin, B. Bakshi. *A Multi-Disciplinary Framework for Sustainable Process Synthesis: Linking Experiments, Molecular Dynamic Simulation, Process Optimization, and Life Cycle Analysis.* 2025 AIChE Annual Meeting.
- S. Ma, L.-C. Lin, B. Bakshi. *A Multi-Scale Computational Study of Hydrolytic Degradation of PET Under Working Condition.* 2025 AIChE Annual Meeting.

# 💻 Internships
- *2025.07 - 2025.08*, Research/Strategy Intern, [Mitsubishi Chemical Group](https://www.mcgc.com/), Japan & The Global KAITEKI Center, USA. Designed an enterprise circularity-metric (CMI) model to inform R&D prioritization; built chemical/process databases for TEA/LCA; ran molecular simulations and kinetic models for end-of-life evaluation; extended a quantum–classical optimization prototype within an MILP backbone.

# 📖 Educations
- *2021 - Present*, Ph.D. Candidate, Chemical Engineering, The Ohio State University, Columbus, OH. (Graduate minor: Computer Science and Engineering, ML track, 2023–2025)
- *2020 - 2021*, Research Assistant, Institute of Catalysis, Zhejiang University, China.
<!-- TODO: add your bachelor's degree here, e.g.: -->
<!-- - *20XX - 2020*, B.S. in XXX, XXX University, China. -->

# 👨‍🏫 Teaching
- *2023 SP*, Graduate Teaching Assistant, CBE 3521 — Transport 2, The Ohio State University.
- *2022 AU*, Graduate Teaching Assistant, CBE 4760 — Process Design 1, The Ohio State University.

# 🎓 Services
- Educational outreach: designed an AP Environmental Science course module for the NSF EFRI E3P project (SP 2024).
